...
{
    // Allocate new memory for the array with one additional element
...
T SimpleVector<T>::popFrnt()
{
    ...
    T removedValue = aptr[0];

    // Allocate new memory for the array with one less element
    T *newArray = new T[arraySize - 1];
...
T SimpleVector<T>::popBack()
{
    ...
    // Store the value to be removed
    T removedValue = aptr[arraySize - 1];
